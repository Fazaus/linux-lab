...
# Verifica existencia, antiguedad y tamano de backups.
...
readonly MIN_TAMANIO_MB=10       # alertar si el backup es mas pequeno
...
uso() {
    ...
    echo "  directorio_backup   Directorio a verificar (por defecto: /backup)"
    echo ""
    echo "Opciones:"
...
log() {
    ...
    # Escribir en pantalla y en el archivo log al mismo tiempo
...
# === Verificacion 4: tamano ===
verificar_tamanio() {
    log "INFO" "Verificando tamano del directorio de backups..."
    
    local tamanio_mb
    tamanio_mb=$(du -sm "$DIR_BACKUP" | awk '{print $1}')
    
    log "INFO" "Tamano total: ${tamanio_mb} MB"
    
    if [ "$tamanio_mb" -lt "$MIN_TAMANIO_MB" ]; then
        log "WARNING" "Directorio pequeno: ${tamanio_mb} MB (minimo: ${MIN_TAMANIO_MB} MB)"
        return 0
    fi
    ...
    log "OK" "Tamano dentro del rango: ${tamanio_mb} MB"
    return 0
}
...
case "$estado_global" in
    OK)
        ...
        ;;
    WARNING)
        log "WARNING" "RESULTADO: verificacion con advertencias."
        exit 0
        ;;
    ERROR)
        log "ERROR" "RESULTADO: errores que requieren atencion."
        exit 1
        ;;
esac
