...
|---|---|
$(cut -d'|' -f2 "$LOGFILE" | tr -d ' ' | sort | uniq -c | sort -rn | head -10 | awk '{printf "| %d | %s |\n", $1, $2}')
...
| Hora | Eventos |
|---|---|
$(cut -d'|' -f1 "$LOGFILE" | awk '{print $2}' | cut -d':' -f1 | sort | uniq -c | awk '{printf "| %s:00 | %d |\n", $2, $1}')

## 4. Top 5 Mensajes de Error

| Frecuencia | Mensaje |
|---|---|
$(grep -E "\| (ERROR|FATAL) \|" "$LOGFILE" | cut -d'|' -f4 | sort | uniq -c | sort -rn | head -5 | awk '{count=$1; $1=""; gsub(/^ +/,"",$0); printf "| %d | %s |\n", count, $0}')

## 5. Resumen

- Sistema analizado con $(wc -l < "$LOGFILE") eventos registrados.
- $(grep -E -c "\| (ERROR|FATAL) \|" "$LOGFILE" 2>/dev/null || echo 0) eventos requieren atencion (ERROR y FATAL).
